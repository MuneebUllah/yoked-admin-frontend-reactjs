...
const Post = ({ reportPostData, setReportPostData }) => {
  ...
  function fetchNextRecords(offset) {
    reportPost(setReportPostData, limit, offset)
  }
  ...
  return (
    <div className=" w-full flex-wrap gap-4 pb-2 px-4 relative">
      ...
      {
        reportPostData?.reportedPosts &&
        reportPostData?.reportedPosts?.map((post , index) => {
          return (
            <div key={index}>
              <div
                className="bg-white  shadow-2xl rounded-3xl w-[320px] h-[430px] my-2 p-4 cursor-pointer"
                onClick={() => {
                  navigate("/reports/post", { state: { id: post?.id } });
                }}
              >
                <img
                  src={post?.postImages[0].image ? post?.postImages[0].image : dummyImage}
                  alt="post "
      ...
      <Fragment >
        <Pagination totalRecords={reportPostData?.count} perPage={limit} handleFetchNextRecords={fetchNextRecords} />
      </Fragment>
    </div>
    </div>
  );
